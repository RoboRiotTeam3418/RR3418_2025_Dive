...
public class Limelight{
  ...
  public void SetPipeline(int Pipelinenum) { // void prevents function from sending back a value.
    ...
    LIMELIGHT2.getEntry("pipeline").setNumber(Pipelinenum); // Sets pipline number
  }
  ...
  public int whichLimelightSees(){
    if (l1_tv.getDouble(0) == 1){ // Unnamed sees
      return 1;
    } else if (l2_tv.getDouble(0) == 1){ // Limeone sees
      return -1;
    } else if (l1_tv.getDouble(0) == 1 && l2_tv.getDouble(0) == 1) { // both (somehow) see

      if ((Math.abs(l1_ta.getDouble(0)) - Math.abs(l2_ta.getDouble(0))) >= 0 ) { // Closer apriltag takes prioity
        return 1;
      } else {
        return -1;
      }
